...
SELECT count(ssn), department FROM employees GROUP BY department HAVING count(*) > 2;
SELECT name FROM departments ORDER BY budget DESC LIMIT 1 OFFSET 1;
SELECT name, lastname FROM employees GROUP BY employees.name,lastname,department HAVING department = 37;
SELECT name FROM employees WHERE city = 'Almaty' UNION SELECT name from customers WHERE city = 'Almaty'; 
SELECT name, code FROM departments WHERE budget > 60000  ORDER BY budget ASC, code DESC;
